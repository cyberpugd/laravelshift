...
<section class="content-header">
     <span style="font-size: 24px;">@{{form.name}} - <a href="@{{form.url}}">@{{form.url}}</a></span>
     <div class="btn-group pull-right">
          <span class="btn btn-sm btn-default" title="Copy to new form" @click="copyForm" v-bind:class="{disabled: copying}">
               <span v-show="!copying"><i class="fa fa-copy"></i></span>
               <span v-show="copying"><i class="fa fa-cog fa-spin"></i></span>
          </span>
     </div>
</section>
...
{{--      <div class="alert alert-danger" role="alert" v-if="errors.length || serverErrors != ''" transition="fade">
        ...
        <div class="row">
                    ...
                    <select class="selectpicker form-control" 
                        ...
                        <optgroup v-for="category in categories" label="@{{ category.name }}">
                                <option v-for="subcategory in category.subcategories" 
                                    ...
                                    :selected="(form.subcategory_id == subcategory.id ? true:false)">
                                        @{{ subcategory.name }}
                                    </option>
                        </optgroup>
            ...
            <div class="col-md-2 form-group" :class="{'has-error' : serverErrors['share_with'] }">
                         ...
                         v-model="form.share_with"
                         multiple>                                   
                                   <option v-for="user in users" 
                                        :value="user" 
                                        data-tokens="@{{user.first_name}} @{{user.last_name}}"
                                       :selected="isSharedWith(user)"
                                        >@{{user.first_name}} @{{user.last_name}}</option>
                                   
                         </select> 
                         <span class="small text-danger" v-text="serverErrors['share_with']"></span>
                    </div>
        </div>
        ...
        <table class="table table-striped" v-if="form.fields.length">
            ...
            <thead>
                <td width="5%"><strong>Row #</strong></td>
                ...
                <tr v-for="(index, field) in form.fields">
                    ...
                    <td>
                         <div class="form-group" :class="{'has-error' : serverErrors['fields.'+index+'.type'] }">
                             <select class="form-control" v-model="field.type" @change="changeType(field, index)" required @click="serverErrors['fields.'+index+'.type'] = ''">
                    ...
                    <td align="center">
                        <input type="radio" name="ticket_subject" 
                             :disabled="field.type == 'checkbox' ? true : false"
                             :value="field.name"
                             :checked="field.ticket_subject == 1 ? true : false"
                             @click="updateSubjectRadio(field)" 
                             v-model="field.ticket_subject_text"
                    ...
                    <td align="center">
                        <input type="radio" 
                             ...
                             :checked="field.ticket_description == 1 ? true : false"
                             @click="updateDescriptionRadio(field)" 
                             v-model="field.ticket_description_text"
                    ...
                    <td align="center">
                         <div class="form-group" :class="{'has-error' : serverErrors['fields.'+index+'.default_value'] }">
                             ...
                             v-if="((field.type == 'text' || field.type == 'select' || field.type == 'number' || field.type == 'hidden') && field.ticket_description == false) || field.type == 'select' || field.type == 'number' || field.type == 'text'"
                             @keyup="serverErrors['fields.'+index+'.default_value'] = ''">
                             <textarea rows="5" class="form-control"
                              v-model="field.default_value"
                              v-on:keyup="stopTyping(field)" required
                              @keydown="serverErrors['fields.'+index+'.label'] = ''" 
                              v-if="((field.type == 'hidden' || field.type == 'textbox') && field.ticket_description == true) || field.type == 'textbox'"
                              ></textarea>
        ...
        <a type="button" class="btn btn-large btn-success"
                    ...
                    v-show="form.fields.length">
                        ...
                        <span v-if="!submitting">Save Form</span></a>
    </div><!-- End panel-body -->
</div>
</section>
</div>
...
<script>
    new Vue({
        ...
        beforeCreate: function() {
               this.form.ticket_description = '';
               this.form.ticket_subject = '';
        },
        methods: {
            addField: function() {
                this.form.fields.push({label: '', type: '', is_required: true, ticket_subject: '', ticket_description: '', default_value: '', name: ''});
            },
            ...
            isSharedWith: function(user) {
               returnVal = false;
               this.form.share_with.filter(function(share) {
                    if(share.id == user.id) {
                         returnVal = true;
                    }
               });
            ...
            saveForm: function() {
                ...
                if(!this.errors.length) {
                    this.$http.post('/admin/forms/'+this.form.id, this.form).then(function(response) {
            ...
            copyForm: function() {
               ...
               this.validate();
               if(!this.errors.length) {
                    this.$http.post('/admin/forms/copy', this.form).then(function(response) {
                         data = JSON.parse(response.body);
                         swal({   
                                   title: '',   
                                   text: data.name+" copied Successfully",   
                                   type: "success",
                                   showConfirmButton: true
                              });
                         this.serverErrors = '';
            ...
            updateSubjectRadio: function(currentField) {
                this.form.fields.forEach(function(field) {
                    field.ticket_subject = false;
                });
                currentField.ticket_subject = true;
            },
            updateDescriptionRadio: function(currentField) {
                this.form.fields.forEach(function(field) {
            ...
            validate: function() {  
                //Provide simple validation for radio buttons and default value
                self = this;
                this.errors = [];
                if(this.ticket_subject == '') {
                    this.errors.push({"ticket_subject": "At least one field must be set as the ticket Subject."});
                }
                if(this.ticket_description == '') {
                    this.errors.push({"ticket_description": "At least one field must be set as the ticket description."});
                }
            }
        }
    });
